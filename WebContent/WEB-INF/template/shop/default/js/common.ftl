...
function getUrlParam(key){
	...
	for(var i=0;i<strArray.length;i++){
		 var keyValueArray=strArray[i].split("=");
		 if(key==keyValueArray[0]){
...
(function($) {
	...
	$.redirectLogin = function (redirectUrl, message) {
		//var href = "${exchangeUrl}/login.html";
	...
	$.fn.extend({
		...
		uploader: function(options) {
			...
			return this.each(function() {
				...
				var webUploader = WebUploader.create({
					swf: '${base}/resources/shop/${theme}/flash/webuploader.swf',
					...
					switch(type) {
						case "F_EXCEED_SIZE":
							$.message("warn", "${message("shop.upload.sizeInvalid")}");
							break;
						case "Q_TYPE_DENIED":
							$.message("warn", "${message("shop.upload.typeInvalid")}");
							break;
						default:
							$.message("warn", "${message("shop.upload.error")}");
					}
				});
		...
		editor: function(options) {
			window.UEDITOR_CONFIG = {
				UEDITOR_HOME_URL: '${base}/resources/shop/${theme}/ueditor/',
				serverUrl: '${base}/file/uploader.jhtml',
...
$().ready(function() {
	...
	$(document).ajaxSend(function(event, request, settings) {
		if (!settings.crossDomain && settings.type != null && settings.type.toLowerCase() == "post") {
			request.setRequestHeader("wxtype", 'wx');
