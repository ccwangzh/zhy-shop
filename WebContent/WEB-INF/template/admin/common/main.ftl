...
$().ready(function() {
	...
	var $headerLogoutA=$("#headerLogout");
	$headerLogoutA.on("click",function(){
		$.ajax({
			url:"/shop/admin/loginOut.jhtml",
			type: "POST",
			dataType: "json",
			cache: false,
			success: function() {
...
<body>
	...
	<table class="main">
		<tr>
			<th class="logo">
				<a href="main.jhtml">
					<img style="margin: 6px 0 0 17px;" src="${base}/resources/admin/images/admin_header.png" alt="比捷商城" />
				</a>
			</th>
			<th>
				<div id="nav" class="nav">
					<ul>
						[#list ["admin:store","admin:storeCategory","admin:storeRank","admin:cash","admin:categoryApplication"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
									<a href="#shop">${message("admin.main.storeNav")}</a>
								</li>
								[#break /]
							[/@shiro.hasPermission]
						[/#list]
						[#list ["admin:goods", "admin:stock", "admin:productCategory", "admin:parameter", "admin:attribute", "admin:specification", "admin:brand", "admin:productNotify:"] as permission]
							...
							[/@shiro.hasPermission]
						[/#list]
						[#list ["admin:order", "admin:orderPayment", "admin:refunds", "admin:shipping", "admin:returns", "admin:deliveryCenter", "admin:deliveryTemplate"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
							...
							[/@shiro.hasPermission]
						[/#list]
						[#list ["admin:member", "admin:memberRank", "admin:memberAttribute","admin:businessAttribute", "admin:point", "admin:deposit", "admin:review", "admin:consultation", "admin:messageConfig"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
						...
						[#list ["admin:statistics", "admin:memberStatistic", "admin:orderStatistic", "admin:memberRanking", "admin:goodsRanking"] as permission]
							...
							[/@shiro.hasPermission]
						[/#list]
						[#list ["admin:setting", "admin:area", "admin:paymentMethod", "admin:shippingMethod", "admin:deliveryCorp", "admin:paymentPlugin", "admin:storagePlugin", "admin:loginPlugin", "admin:promotionPlugin", "admin:admin", "admin:role", "admin:message", "admin:log"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
						...
						[/#list]
						<li>
							<a href="${basePath}/" target="_blank">${message("admin.main.home")}</a>
						</li>
					</ul>
				</div>
				<div class="link">
				...
				<div class="link">
					...
					<a id="headerLogout" target="_top" class="logOut">[${message("admin.main.logout")}]</a>
				</div>
			</th>
		</tr>
		<tr>
			<td id="menu" class="menu">
				<dl id="shop" class="default">
					<dt>${message("admin.main.store")}</dt>
					[@shiro.hasPermission name="admin:store"]
						<dd>
							<a href="../store/list.jhtml" target="iframe">${message("admin.main.store")}</a>
						</dd>
					[/@shiro.hasPermission]
					[@shiro.hasPermission name="admin:storeCategory"]
						<dd>
							<a href="../store_category/list.jhtml" target="iframe">${message("admin.main.storeCategory")}</a>
						</dd>
					[/@shiro.hasPermission]
					[@shiro.hasPermission name="admin:storeRank"]
						<dd>
							<a href="../store_rank/list.jhtml" target="iframe">${message("admin.main.storeRank")}</a>
						</dd>
					[/@shiro.hasPermission]
					[@shiro.hasPermission name="admin:cash"]
						<dd>
							<a href="../cash/list.jhtml" target="iframe">${message("admin.main.cash")}</a>
						</dd>
					[/@shiro.hasPermission]
					[@shiro.hasPermission name="admin:categoryApplication"]
						<dd>
							<a href="../category_application/list.jhtml" target="iframe">${message("admin.main.categoryApplication")}</a>
						</dd>
					[/@shiro.hasPermission]
				</dl>
				<dl id="product">
				...
				<dl id="order">
					...
					[@shiro.hasPermission name="admin:orderPayment"]
						<dd>
							<a href="../order_payment/list.jhtml" target="iframe">${message("admin.main.orderPayment")}</a>
						</dd>
					[/@shiro.hasPermission]
				...
				<dl id="member">
					...
					[@shiro.hasPermission name="admin:businessAttribute"]
						<dd>
							<a href="../business_attribute/list.jhtml" target="iframe">${message("admin.main.businessAttribute")}</a>
						</dd>
					[/@shiro.hasPermission]
				...
				<dl id="system">
					...
					[@shiro.hasPermission name="admin:promotionPlugin"]
						<dd>
							<a href="../promotion_plugin/list.jhtml" target="iframe">${message("admin.main.promotionPlugin")}</a>
						</dd>
					[/@shiro.hasPermission]
