...
public class CcbPaymentPlugin extends PaymentPlugin {
	...
	public boolean verifyNotify(PaymentPlugin.NotifyMethod notifyMethod, HttpServletRequest request) {
		...
		if (paymentTransaction != null && verifySign(signMap, request.getParameter("SIGN")) && "Y".equals(request.getParameter("SUCCESS")) && paymentTransaction.getAmount().compareTo(new BigDecimal(request.getParameter("PAYMENT"))) == 0) {
			return true;
		}
		return false;
	}
