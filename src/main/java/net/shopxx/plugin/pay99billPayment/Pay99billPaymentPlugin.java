...
public class Pay99billPaymentPlugin extends PaymentPlugin {
	...
	public boolean verifyNotify(PaymentPlugin.NotifyMethod notifyMethod, HttpServletRequest request) {
		PluginConfig pluginConfig = getPluginConfig();
		PaymentTransaction paymentTransaction = getPaymentTransaction(request.getParameter("orderId"));
		Map<String, Object> parameterMap = new LinkedHashMap<String, Object>();
		...
		if (paymentTransaction != null && generateSign(parameterMap).equals(request.getParameter("signMsg")) && pluginConfig.getAttribute("partner").equals(request.getParameter("merchantAcctId")) && "10".equals(request.getParameter("payResult"))
