...
public class AbcPaymentPlugin extends PaymentPlugin {
	...
	public boolean verifyNotify(PaymentPlugin.NotifyMethod notifyMethod, HttpServletRequest request) {
		try {
			...
			PaymentTransaction paymentTransaction = getPaymentTransaction(getSn(request));
			if (paymentTransaction != null && verifySign(message) && pluginConfig.getAttribute("merchantId").equals(merchantId) && "0000".equals(returnCode) && paymentTransaction.getAmount().compareTo(new BigDecimal(amount)) == 0) {
				return true;
			}
			return false;
