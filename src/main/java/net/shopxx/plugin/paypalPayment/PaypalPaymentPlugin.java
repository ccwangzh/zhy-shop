...
public class PaypalPaymentPlugin extends PaymentPlugin {
	...
	public boolean verifyNotify(PaymentPlugin.NotifyMethod notifyMethod, HttpServletRequest request) {
		PluginConfig pluginConfig = getPluginConfig();
		PaymentTransaction paymentTransaction = getPaymentTransaction(request.getParameter("invoice"));
		if (paymentTransaction != null && pluginConfig.getAttribute("partner").equals(request.getParameter("receiver_email")) && pluginConfig.getAttribute("currency").equals(request.getParameter("mc_currency")) && "Completed".equals(request.getParameter("payment_status"))
