...
public class YeepayPaymentPlugin extends PaymentPlugin {
	...
	public boolean verifyNotify(PaymentPlugin.NotifyMethod notifyMethod, HttpServletRequest request) {
		PluginConfig pluginConfig = getPluginConfig();
		PaymentTransaction paymentTransaction = getPaymentTransaction(request.getParameter("r6_Order"));
		...
		if (paymentTransaction != null && generateSign(parameterMap).equals(parameterValuesMap.get("hmac")) && pluginConfig.getAttribute("partner").equals(parameterValuesMap.get("p1_MerId")) && "1".equals(parameterValuesMap.get("r1_Code"))
