...
public class ReviewDaoImpl extends BaseDaoImpl<Review, Long> implements ReviewDao {
	...
	public Page<Review> findPage(Member member, Goods goods, Store store, Review.Type type, Boolean isShow, Pageable pageable) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Review> criteriaQuery = criteriaBuilder.createQuery(Review.class);
		Root<Review> root = criteriaQuery.from(Review.class);
		criteriaQuery.select(root);
	...
	public long calculateTotalScore(Goods goods) {
		...
		String jpql = "select sum(review.score) from Review review where review.goods = :goods and review.isShow = :isShow and review.forReview is null";
	...
	public long calculateScoreCount(Goods goods) {
		...
		String jpql = "select count(*) from Review review where review.goods = :goods and review.isShow = :isShow and review.forReview is null";
