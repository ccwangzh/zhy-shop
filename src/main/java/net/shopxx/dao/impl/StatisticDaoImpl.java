...
public class StatisticDaoImpl extends BaseDaoImpl<Statistic, Long> implements StatisticDao {
	...
	public boolean exists(Store store, int year, int month, int day) {
		...
		String jpql;
		if (store == null) {
			jpql = "select count(*) from Statistic statistic where statistic.year = :year and statistic.month = :month and statistic.day = :day and statistic.store is null";
			count = entityManager.createQuery(jpql, Long.class).setParameter("year", year).setParameter("month", month).setParameter("day", day).getSingleResult();
		} else {
			jpql = "select count(*) from Statistic statistic where statistic.year = :year and statistic.month = :month and statistic.day = :day and statistic.store = :store";
			count = entityManager.createQuery(jpql, Long.class).setParameter("year", year).setParameter("month", month).setParameter("day", day).setParameter("store", store).getSingleResult();
		}
		return count > 0;
	}
