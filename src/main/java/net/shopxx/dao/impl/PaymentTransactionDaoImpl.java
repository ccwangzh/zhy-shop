 ...
 * @version 5.0
 */
@Repository("paymentTransactionDaoImpl")
public class PaymentTransactionDaoImpl extends BaseDaoImpl<PaymentTransaction, Long> implements PaymentTransactionDao {
	...
	public PaymentTransaction findBySn(String sn) {
		...
		String jpql = "select paymentTransaction from PaymentTransaction paymentTransaction where lower(paymentTransaction.sn) = lower(:sn)";
		try {
			return entityManager.createQuery(jpql, PaymentTransaction.class).setParameter("sn", sn).getSingleResult();
	...
	public PaymentTransaction findAvailableParent(Collection<PaymentTransaction.LineItem> lineItems, PaymentPlugin paymentPlugin) {
		...
		for (LineItem lineItem : lineItems) {
			...
			case SVC_PAYMENT:
				...
				break;
			case DEPOSIT_RECHARGE:
				subqueryRestrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(subqueryRoot.get("member"), lineItem.getTarget()));
				break;
			case BAIL_PAYMENT:
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("store"), lineItem.getTarget()));
				break;
