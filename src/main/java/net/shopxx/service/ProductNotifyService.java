...
public interface ProductNotifyService extends BaseService<ProductNotify, Long> {
	 ...
	 * @return 到货通知分页
	 */
	Page<ProductNotify> findPage(Store store, Member member, Goods.Status status, Boolean isOutOfStock, Boolean hasSent, Pageable pageable);
	 ...
	 * @return 到货通知数量
	 */
	Long count(Member member,Goods.Status status, Boolean isOutOfStock, Boolean hasSent);
