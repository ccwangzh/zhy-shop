...
public class SearchServiceImpl implements SearchService {
	...
	public void index(Class<?> type) {
		...
		if (type == Article.class) {
			...
			for (int i = 0; i < storeDao.count(); i += 20) {
				List<Store> storeList = storeDao.findList(i, 20, null, null);
				for (Store store : storeList) {
					fullTextEntityManager.index(store);
				}
				fullTextEntityManager.flushToIndexes();
				fullTextEntityManager.clear();
				storeDao.clear();
	...
	public void purge(Class<?> type) {
		...
		if (type == Article.class) {
			...
			fullTextEntityManager.purgeAll(Store.class);
		}
	}
	...
	public void purge(Store store) {
		if (store != null) {
			FullTextEntityManager fullTextEntityManager = Search.getFullTextEntityManager(entityManager);
			fullTextEntityManager.purge(Store.class, store.getId());
		}
	}
	...
	public Page<Article> search(String keyword, Pageable pageable) {
		...
		try {
			...
			Query titleQuery = titleParser.parse(QueryParser.escape(keyword));
			FuzzyQuery titleFuzzyQuery = new FuzzyQuery(new Term("title", keyword), FUZZY_QUERY_MINIMUM_SIMILARITY);
			...
			fullTextQuery.setSort(new Sort(new SortField[] { new SortField("isTop", SortField.STRING, true), new SortField(null, SortField.SCORE), new SortField("createdDate", SortField.LONG, true) }));
	...
	public Page<Goods> search(String keyword, BigDecimal startPrice, BigDecimal endPrice, Goods.OrderType orderType, Pageable pageable) {
		...
		try {
			TermQuery snQuery = new TermQuery(new Term("sn", keyword));
			Query keywordQuery = new QueryParser(Version.LUCENE_36, "keyword", new IKAnalyzer()).parse(QueryParser.escape(keyword));
			...
			Query nameQuery = nameParser.parse(QueryParser.escape(keyword));
			FuzzyQuery nameFuzzyQuery = new FuzzyQuery(new Term("name", keyword), FUZZY_QUERY_MINIMUM_SIMILARITY);
			TermQuery introductionQuery = new TermQuery(new Term("introduction", keyword));
			TermQuery isMarketableQuery = new TermQuery(new Term("status", Goods.Status.onTheshelf.ordinal()+""));
			...
			if (orderType != null) {
				switch (orderType) {
				case topDesc:
					sortFields = new SortField[] { new SortField("isTop", SortField.STRING, true), new SortField(null, SortField.SCORE), new SortField("createdDate", SortField.LONG, true) };
					break;
				case priceAsc:
					sortFields = new SortField[] { new SortField("price", SortField.DOUBLE, false), new SortField("createdDate", SortField.LONG, true) };
					break;
				case priceDesc:
					sortFields = new SortField[] { new SortField("price", SortField.DOUBLE, true), new SortField("createdDate", SortField.LONG, true) };
					break;
				case salesDesc:
					sortFields = new SortField[] { new SortField("sales", SortField.LONG, true), new SortField("createdDate", SortField.LONG, true) };
					break;
				case scoreDesc:
					sortFields = new SortField[] { new SortField("score", SortField.FLOAT, true), new SortField("createdDate", SortField.LONG, true) };
					break;
				case dateDesc:
					sortFields = new SortField[] { new SortField("createdDate", SortField.LONG, true) };
					break;
				}
			} else {
				sortFields = new SortField[] { new SortField("isTop", SortField.STRING, true), new SortField(null, SortField.SCORE), new SortField("createdDate", SortField.LONG, true) };
	...
	public List<Store> searchStore(String keyword) {
		...
		try {
			...
			nameParser.setDefaultOperator(QueryParser.AND_OPERATOR);
			Query nameQuery = nameParser.parse(QueryParser.escape(keyword));
			FuzzyQuery nameFuzzyQuery = new FuzzyQuery(new Term("name", keyword));
			Query statusQuery = new TermQuery(new Term("status", "success"));
			Query isEnabledQuery = new TermQuery(new Term("isEnabled", "true"));
			BooleanQuery textQuery = new BooleanQuery();
			BooleanQuery query = new BooleanQuery();
			textQuery.add(nameQuery, BooleanClause.Occur.SHOULD);
			textQuery.add(nameFuzzyQuery, BooleanClause.Occur.SHOULD);
			query.add(statusQuery, BooleanClause.Occur.MUST);
			query.add(isEnabledQuery, BooleanClause.Occur.MUST);
			query.add(textQuery, BooleanClause.Occur.MUST);
			FullTextEntityManager fullTextEntityManager = Search.getFullTextEntityManager(entityManager);
			FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(query, Store.class);
			fullTextQuery.setSort(new Sort(new SortField[] { new SortField(null, SortField.SCORE), new SortField("createdDate", SortField.LONG, true) }));
