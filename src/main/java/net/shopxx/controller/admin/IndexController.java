...
public class IndexController extends BaseController {
	...
	public @ResponseBody Map<String, Object> generate(IndexController.GenerateType generateType, Boolean isPurge, Integer first, Integer count) {
		...
		case store:
			if (first == 0 && isPurge != null && isPurge) {
				searchService.purge(Store.class);
			}
			List<Store> storeList = storeService.findList(first, count, null, null);
			for (Store store : storeList) {
				searchService.index(store);
				generateCount++;
			}
