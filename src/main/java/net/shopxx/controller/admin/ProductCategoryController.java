...
public class ProductCategoryController extends BaseController {
	 ...
	 * 促销选择
	 */
	@RequestMapping(value = "/promotion_select", method = RequestMethod.GET)
	public @ResponseBody List<Map<String, Object>> promotionSelect(@RequestParam("q") String keyword, Long[] excludeIds, @RequestParam("limit") Integer count) {
		List<Map<String, Object>> data = new ArrayList<Map<String, Object>>();
		if (StringUtils.isEmpty(keyword)) {
			return data;
		}
		Set<Promotion> excludes = new HashSet<Promotion>(promotionService.findList(excludeIds));
		List<Promotion> promotions = promotionService.search(keyword, excludes, count);
	...
	public String update(ProductCategory productCategory, Long parentId, Long[] brandIds, Long[] promotionIds, RedirectAttributes redirectAttributes) {
		...
		productCategoryService.update(productCategory, "stores", "categoryApplications", "treePath", "grade", "children", "goods", "parameters", "attributes", "specifications");
		addFlashMessage(redirectAttributes, SUCCESS_MESSAGE);
		return "redirect:list.jhtml";
	}
