...
public class Order extends BaseEntity<Long> {
	...
	/** 订单是否为挂牌交易,此属性只记录当前订单是否是挂牌交易来配送*/
	private Boolean isListedtrade = false;
	...
	public boolean canDelete() {
		if (Order.Status.completed.equals(getStatus()) || getAmountPaid().compareTo(BigDecimal.ZERO) <= 0 && (Order.Status.canceled.equals(getStatus()) || Order.Status.failed.equals(getStatus()) || Order.Status.denied.equals(getStatus()))) {
			return true;
		}
		return false;
	}
