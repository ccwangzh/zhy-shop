...
public class Goods extends BaseEntity<Long> {
	...
	/** 是否启用通用协议 */
	private Boolean isEnableCommonAgreement;
	...
	/** 协议标题*/
	private String agreementTitile;
	/**
	 * 获取编号
	 * 
	 * @return 编号
    ...
    @NotNull(groups = Save.class)
    @Column(nullable = false)
	public Goods.Status getStatus() {
        return status;
    }

    public void setStatus(Goods.Status status) {
        this.status = status;
    }
	...
	public Boolean getIsMarketable() {
		return getStatus()==Goods.Status.onTheshelf;
	}
	 ...
	 * @return 是否启用通用协议
	 */
	public Boolean getIsEnableCommonAgreement() {
		return isEnableCommonAgreement;
	}

	/**
	 * 设置是否启用通用协议
	 *
	 * @param isEnableCommonAgreement
	 *            是否启用通用协议
	 */
	public void setIsEnableCommonAgreement(Boolean isEnableCommonAgreement) {
		this.isEnableCommonAgreement = isEnableCommonAgreement;
	}
	 ...
	 */
	@ManyToMany(fetch = FetchType.LAZY)
	@JoinTable(name = "xx_store_goods_tag")
	@OrderBy("order asc")
     ...
     */
    @Length(max = 200)
    public String getAgreementTitile() {
        return agreementTitile;
    }

    /**
     * 协议标题
     * @param agreementTitile
     */
    public void setAgreementTitile(String agreementTitile) {
        this.agreementTitile = agreementTitile;
    }
	...
	public String getPath() {
		...
		return templateConfig.resolveStaticPath(model);
	}
	...
	public boolean isValid(Promotion promotion) {
		if (!Goods.Type.general.equals(getType()) || !promotion.getIsEnabled() || !promotion.hasBegun() || promotion.hasEnded() || CollectionUtils.isEmpty(promotion.getMemberRanks())) {
			return false;
		}
